...
//排除特殊情况
bool judgeFormula(string formula)
{
	if (formula == "()" || formula == "（）" || formula == "(" || formula == ")")
	{
		cout << "不符合要求，请输入命题公式" << endl;
		return 0;
	}
	return 1;
}
...
int main() 
{
	...
	if (!judgeFormula(formula))
		return 0;
	Map_ic proposition_set = getProposition(formula);//建立命题变项
...
int findProposition(Map_ic pSet, char p) //返回-1，表示该命题变项尚未被遍历过，可计数；否则说明该命题变项已被遍历过，则不重复计数。另外，还可以返回指定命题变项的下标的位置
{
	Map_ic::iterator it = pSet.begin();
	while (it != pSet.end()) 
	{
		if (it->second == p) //若该命题变项出现过，则返回
		{
			return it->first;
		}
		it++;
